...
2.  **测量过程**:
    1.  **准备反应**: 准备好反应溶液。
    2.  **开始计时**: 当反应物混合时（例如混合到一半时），立即点击 **【开始计时】**。此时您需要选择一个 `.xlsx` 文件路径用于自动保存本次实验的数据。注意，计时将在你**成功提交**路径名（你需要先解决路径选择、命名和文件名重复等问题）之后立刻开始。
    3.  反应液快速装入旋光管中，放回仪器，迅速调整好视场后，点击 **【开始跟踪】**。
    4.  系统将自动进行测量，实时跟踪反应过程，并在下方的表格中显示数据点。
    5.  当达到设定的**采样点数目**后，测量会自动停止，数据会自动保存到您之前选定的文件中，并且电机将自动复位到零点。
    6.  在测量过程中可以随时更改**采样点数目**和**步进角度**。
    7.  点击**【停止跟踪】**不会停止计时，机器会自动回到零点可以重新开始跟踪。
    8.  点击 **【停止计时】**也会停止跟踪，而且会停止计时，这一次反应溶液将无法再次被测量。
...
2.  **设置参数**:
    * **α∞**: 输入反应终点的旋光度 `α∞` 值。
    * **拟合模式**: 根据反应级数，选择不同的回归模型（`lnΔα - t` 对应一级反应）。
